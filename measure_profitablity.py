...
message = f"We ran {total_number_of_runs} runs of {num_hands} hands each, using the basic strategy. Of these, {bankrupting_runs} went bankrupt, {losing_runs} lost money, and {profitable_runs} were profitable.\nOn these runs, the average profit was {pnl_basicstrategy.iloc[-1].mean()/(num_hands*args.bet_per_hand)*100} percent of each hand; the maximal profit was {pnl_basicstrategy.iloc[-1].max()/(num_hands*args.bet_per_hand)}"
print(message)
...
message = f"We ran {total_number_of_runs} runs of {num_hands} hands each, using the card counting. Of these, {bankrupting_runs} went bankrupt, {losing_runs} lost money, and {profitable_runs} were profitable.\nOn these runs, the average profit was {pnl_cardcount.iloc[-1].mean()/(num_hands*args.bet_per_hand)*100} percent of each hand; the maximal profit was {pnl_cardcount.iloc[-1].max()/(num_hands*args.bet_per_hand)}"
print(message)